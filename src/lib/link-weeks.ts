...
export function summarizeWeek(week: LinkWeekEntry): WeekSummary {
  ...
  return {
    ...
    topCategories: rankItems(links.map((link) => link.category)).map((item) => item.name),
    topTags: rankItems(links.flatMap((link) => link.tags)).map((item) => item.name),
  }
}

export function groupWeekSummariesByYear(linkWeeks: LinkWeekEntry[]): YearSummary[] {
