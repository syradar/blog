...
const props = defineProps<{ record: LinksExplorerRecord }>()
const emit = defineEmits<{
  categoryClick: [category: LinkCategory]
  tagClick: [tag: string]
}>()

const categoryColor = computed(() => categoryColorTokens[props.record.category])
</script>
  ...
  >
    ...
    <div class="chips-container">
      <button
        ...
        @click="emit('categoryClick', record.category)"
      >
      ...
      <button
        v-for="recordTag in record.tags"
        :key="recordTag"
        type="button"
        class="tag-chip"
        :title="`Filter by tag: ${recordTag}`"
        @click="emit('tagClick', recordTag)"
      >
        {{ recordTag }}
      </button>
    </div>
...
.link-card {
  ...
  padding: 1rem;
  background: color-mix(in oklab, var(--sl-color-bg), var(--sl-color-gray-6) 35%);
  cursor: pointer;
...
.link-card:hover {
  background: color-mix(in oklab, var(--sl-color-bg), var(--sl-color-gray-5) 15%);
}
...
.category-chip {
  ...
  gap: 0.35rem;
  margin: 0;
  border: 1px solid color-mix(in oklab, var(--category-color), var(--sl-color-gray-5) 62%);
  border-radius: 999px;
...
.category-chip:focus-visible {
  background: color-mix(in oklab, var(--sl-color-bg), var(--category-color) 16%);
...
.tag-chip:hover,
.tag-chip:focus-visible {
  background: color-mix(in oklab, var(--sl-color-bg), var(--sl-color-gray-5) 35%);
