...
const safeCategory = computed<LinkCategory>(() =>
  isLinkCategory(props.category) ? props.category : "Other",
)

const categoryIconPaths: Record<LinkCategory, string> = {
  Frontend: "M17.25 6.75 22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3-4.5 16.5",
  Backend:
...
<template>
  <span class="category-icon" :title="safeCategory" :style="{ '--icon-size': props.size }">
    <svg
      ...
      aria-hidden="true"
    >
      <path stroke-linecap="round" stroke-linejoin="round" :d="categoryIconPaths[safeCategory]" />
    </svg>
  </span>
</template>
